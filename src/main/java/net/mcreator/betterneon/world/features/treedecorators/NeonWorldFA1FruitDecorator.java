...
public class NeonWorldFA1FruitDecorator extends CocoaDecorator {
	public static final NeonWorldFA1FruitDecorator INSTANCE = new NeonWorldFA1FruitDecorator();
	public static com.mojang.serialization.Codec<NeonWorldFA1FruitDecorator> codec;
	public static TreeDecoratorType<?> tdt;
	static {
		codec = com.mojang.serialization.Codec.unit(() -> INSTANCE);
		tdt = new TreeDecoratorType<>(codec);
		tdt.setRegistryName("neon_world_fa_1_tree_fruit_decorator");
		ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
	}

	public NeonWorldFA1FruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (random.nextFloat() <= 0.25F) {
						...
						if (Feature.isAir(level, blockpos)) {
							biConsumer.accept(blockpos, BetterNeonModBlocks.NEON_BLOCK_CGH_1.get().defaultBlockState());
						}
					}
				}
			});
