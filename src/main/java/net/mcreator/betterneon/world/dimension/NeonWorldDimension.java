...
public class NeonWorldDimension {
	@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD)
	public static class Fixers {
		@SubscribeEvent
		public static void registerFillerBlocks(FMLCommonSetupEvent event) {
			Set<Block> replaceableBlocks = new HashSet<>();
			replaceableBlocks.add(BetterNeonModBlocks.NEON_BLOCK.get());
			replaceableBlocks.add(BetterNeonModBlocks.NEON_BLOCK_CGH_1.get());
			replaceableBlocks.add(BetterNeonModBlocks.NEON_BLOCK.get());
			replaceableBlocks.add(BetterNeonModBlocks.NEON_BLOCK.get());
			replaceableBlocks.add(BetterNeonModBlocks.NEON_BLOCK.get());
			event.enqueueWork(() -> {
		...
		public static void registerDimensionSpecialEffects(FMLClientSetupEvent event) {
			...
			event.enqueueWork(() -> DimensionSpecialEffects.EFFECTS.put(new ResourceLocation("better_neon:neon_world"), customEffect));
		}
	}
	...
	public static void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getFrom() == ResourceKey.create(Registry.DIMENSION_REGISTRY, new ResourceLocation("better_neon:neon_world"))) {

			SMYS1DangYaoShuiXiaoGuoKaiShiYingYongShiProcedure.execute(world, entity);
		}
		if (event.getTo() == ResourceKey.create(Registry.DIMENSION_REGISTRY, new ResourceLocation("better_neon:neon_world"))) {
